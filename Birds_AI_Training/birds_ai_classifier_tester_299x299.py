...
class BirdAITesterApp:
    ...
    # dann per Blur-Padding quadratisch auf 299x299 gebracht.
    # =====================================================================
    ...
    def __init__(self, root):
        ...
        # Model and Labels
        self.model_path = "my_birds_modell_299x299.keras"
    ...
    def blur_pad_to_square(self, img, target_size=299):
    ...
    def resize_to_square(self, img, target_size=299):
        """Hartes Resize: Bild direkt auf 299x299 skalieren (verzerrt)."""
        return img.resize((target_size, target_size), Image.LANCZOS)

    def edge_pad_to_square(self, img, target_size=299):
    ...
    def crop_to_square(self, img, target_size=299):
    ...
    def preprocess_image(self, file_path):
        ...
        2. Je nach gewähltem Padding-Modus auf 299x299 bringen
        """
        img = Image.open(file_path).convert('RGB')
        
        # Schritt 2: Padding-Modus anwenden
        mode = self.padding_var.get()
        if mode == "Resize 299x299":
            img = self.resize_to_square(img, target_size=299)
        elif mode == "Replicate / Edge-Padding":
            img = self.edge_pad_to_square(img, target_size=299)
        elif mode == "Center-Crop (quadratisch)":
            img = self.crop_to_square(img, target_size=299)
        else:
            # Standard: Blur-Padding
            img = self.blur_pad_to_square(img, target_size=299)
        return img
    ...
    def load_and_classify(self):
        ...
        # Bild vorverarbeiten (Kamera-Resize → Blur-Padding → 299x299)
        try:
